...
$(document).ready(() => {
    ...
    $.ajax({
        ...
        success: (data) => {
            ...
            /* Hieronder wordt er een HTML-code speciaal voor blogartikelen gegenereerd. Er worden alleen eerste drie blogartikelen binnengehaald. */
            for (let i = 3; i > 0; i--) {
                /* Blogartikel */
                blogartikelen += "<div id='blog_" + i + "' class='col-03 achtergrond_roze'>\n";
                /* Afbeelding */
                blogartikelen += "\t<img class='blog_afbeelding' src='../img/" + $("#tabel #" + i + " td:eq(6)").html() + "' title='" + $("#tabel #" + i + " td:eq(1)").html() + "' alt='" + $("#tabel #" + i + " td:eq(7)").html() + "'>\n";
                /* Blog */
                blogartikelen += "\t<section class='inhoud_2'>\n";
                /* Titel */
                blogartikelen += "\t\t<h3>" + $("#tabel #" + i + " td:eq(1)").html() + "</h3>\n";
                /* Inhoud */
                blogartikelen += "\t\t<p class='tekst_verkorten'>" + $("#tabel #" + i + " td:eq(2)").html() + "</p>\n";
                /* Naam, functie en datum */
                blogartikelen += "\t\t<h6>" + $("#tabel #" + i + " td:eq(3)").html() + " &#124; " + $("#tabel #" + i + " td:eq(4)").html() + " &#124; " + $("#tabel #" + i + " td:eq(5)").html() + "</h6>\n";
            ...
            /* Maak van alle blogartikelen een link naar dezelfde detailpagina. */
            $("div[id*='blog_']").on('click', function () {
                /* Er wordt alvast een blog_id van gekozen blogartikel vastgelegd. De functie "substr" "knipt" het getal uit de id. */
                const getal = $(this).attr('id').substr(5);
                /* Laten zien dat er een getal als waarde aanwezig is. */
                /* alert(getal); */
                /* Dit getal wordt lokaal opgeslagen zodat het weer binnengehaald kan worden door de pagina "blog_detailpagina.html". */ 
                localStorage.setItem ('waarde', getal);
                /* De pagina "blog_detailpagina.html" wordt geopend. */
                window.location.href = "blog_detailpagina.html";
            })
            /* De foto vervagen wanneer de muis over de afbeelding ".blog_afbeelding" gaat. */
            $(".blog_afbeelding").mouseenter(function () {
                $(this).animate({opacity: '0.1'}, 250);
            });
            /* De foto verscherpen wanneer de muis van de afbeelding ".blog_afbeelding" gaat. */
            $(".blog_afbeelding").mouseleave(function () {
